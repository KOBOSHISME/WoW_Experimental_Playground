...
The window lists current `party1` through `party4` members. Select one or more party members, choose a bounded duration, optionally add a short custom message, choose whether to include your sender name, then pick from the scrollable prank list. Use All Party to target everyone currently in the party, or None to clear the target selection. Add sound to prank is off by default; when checked, visual and UI pranks also queue the selected sound for the same targets. Each row has a selector, a short description, a type label, and its own Send button. The footer can still send the selected row, clear effects that you sent, or refresh the party list. Clear Me removes active pranks from your own client without sending anything to the party; `/wep pranks clearself` does the same thing from chat. The percent field appears for visual pranks, where it controls intensity.
